...
def save_trained_model(vae, optimizer, model_path,model_name = "", latent_dim = "", beta = "", n_rows_train = "", time = "",epochs = 0, batch = 512, input_dim = 37, AWS = False, s3 = None, BUCKET = ""):
    if AWS:
        ...
        model_path = f"./Resources/Models/{model_name}_EPOCHS{epochs}_LD{latent_dim}_BETA{beta}_NT{n_rows_train}_BATCH{batch}_INPUT{input_dim}_{time}.keras"
        vae.compile(optimizer = optimizer)
        vae.save(model_path)
